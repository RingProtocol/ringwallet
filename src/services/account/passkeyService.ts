...
class PasskeyService {
  ...
  static _parseAuthData(authData: Uint8Array): ParsedAuthData | null {
    try {
      ...
      DbgLog.log(
        '📍 Public Key CBOR byte length:',
        publicKeyBytes.length,
        'bytes'
      )
      DbgLog.log(
        '📍 Public Key CBOR first 20 bytes:',
        Array.from(publicKeyBytes.slice(0, 20))
      )
      ...
      DbgLog.log(
        '📊 Public Key type:',
        publicKey instanceof Map ? 'Map' : typeof publicKey
      )
  ...
  static async verifyIdentity(credentialId: string): Promise<boolean> {
    try {
      ...
      const credentialIdBuffer = bytes.buffer as ArrayBuffer
  ...
  static async signChallenge(
    credentialId: string | number[] | ArrayBuffer | Uint8Array | DataView,
      ...
      if (typeof credentialId === 'string') {
        try {
          ...
          try {
            ...
            throw new Error(
              `Failed to decode credentialId string: ${(e2 as Error).message}`,
              { cause: e2 }
            )
          }
        }
        ...
        throw new Error(
          `Invalid credentialId type: ${typeof credentialId}. Expected string, array, ArrayBuffer, or ArrayBufferView.`
        )
      }
  ...
  static async isSupported(): Promise<boolean> {
    ...
    if (!PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable) {
      console.warn(
        '❌ isUserVerifyingPlatformAuthenticatorAvailable() is not available'
      )
      this.#supportCache = false
      return false
    }
  ...
  static async register(
    username: string,
    existingSeed: Uint8Array | null = null
    ...
    try {
      ...
      if (usernameBytes.length > maxUsernameBytes) {
        while (
          usernameBytes.length > maxUsernameBytes &&
          usernameToUse.length > 0
        ) {
      ...
      try {
        ...
        const decodedAttestation = decode(attestationObject) as {
          authData: Uint8Array | ArrayBuffer
        }
        DbgLog.log('✅ attestationObject decoded')
        DbgLog.log(
          '📊 decodedAttestation keys:',
          Object.keys(decodedAttestation)
        )
        ...
        DbgLog.log(
          '📊 authData length:',
          (authData as Uint8Array)?.length ||
            (authData as ArrayBuffer)?.byteLength ||
            'unknown'
        )
        ...
        if (publicKey) {
          const keyData = CharUtils.coseKeyToStorage(
            publicKey as Map<number, Uint8Array>
          )
          if (keyData) {
            const credentialIdBase64 = CharUtils.uint8ArrayToBase64(
              new Uint8Array(credential.rawId)
            )
            if (credentialIdBase64) {
              safeSetItem(
                `new_wallet_pk_${credentialIdBase64}`,
                JSON.stringify(keyData)
              )
              DbgLog.log(
                '💾 EIP-7951 Public Key saved with key:',
                `new_wallet_pk_${credentialIdBase64}`
              )
            } else {
  ...
  static async login(isConditional = false): Promise<LoginResult> {
    try {
      ...
      try {
        ...
        const storageKey = credentialIdBase64
          ? `new_wallet_pk_${credentialIdBase64}`
          : null
      ...
      if (response.userHandle) {
        try {
          ...
          if (
            ...
            console.warn(
              'UserHandle contains control characters, ignoring:',
              username
            )
            username = null
          }
    ...
    try {
      ...
      if (isApiAvailable) {
        ...
        if (results[1].status === 'fulfilled') {
          ...
          console.warn(
            'Conditional mediation check failed (non-critical):',
            results[1].reason
          )
        }
      }
      ...
      if (!isUVPAAAvailable && isApiAvailable && isIOSWithPasscodeCapable()) {
        DbgLog.log(
          '📱 iOS 16+: UVPAA=false but passcode can be used for passkey; enabling fallback'
        )
        isUVPAAAvailable = true
        isIOSFallback = true
      }
