...
export function useSendForm(initialToken?: SendTokenOption) {
  ...
  const [selectedToken, setSelectedToken] = useState<SendTokenOption>(
    initialToken ?? { type: 'native', symbol: activeChain?.symbol || 'ETH' }
  )

  const tokenOptions: SendTokenOption[] = useMemo(() => {
    const native: SendTokenOption = {
      type: 'native',
      symbol: activeChain?.symbol || 'ETH',
    }
    const imported =
      activeWallet && activeChain
        ? getTokenList(activeWallet.address, activeChain.id).map((t) => ({
            type: 'erc20' as const,
            token: t,
          }))
        : []
