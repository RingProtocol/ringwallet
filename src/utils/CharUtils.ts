...
class CharUtils {
  ...
  static arrayToUint8Array(
    data: ByteLike | Record<string, unknown> | null | undefined
  ): Uint8Array | null {
    if (!data) return null
      ...
      try {
        ...
        if (keys.length > 0) {
          ...
          for (const key of keys) {
            arr[key] =
              typeof obj[key] === 'number' ? obj[key] : Number(obj[key])
          }
          return new Uint8Array(arr)
        }
        return new Uint8Array(
          Object.values(obj).map((v) => (typeof v === 'number' ? v : Number(v)))
        )
      } catch (e) {
        console.warn(
          'CharUtils.arrayToUint8Array: failed to create from object',
          e,
          data
        )
        return null
  ...
  static uint8ArrayToBase64URL(
    data: ByteLike | null | undefined
  ): string | null {
  ...
  static base64URLToUint8Array(
    ...
    try {
      ...
      console.warn(
        'CharUtils.base64URLToUint8Array: Base64URL decode failed',
        e
      )
      return null
    }
  ...
  static coseKeyToStorage(
    coseKey: CoseKey | null | undefined
  ): CoseStorageFormat | null {
    if (!coseKey) {
    ...
    if (coseKey instanceof Map) {
      ...
      if (!x || !y) {
        console.warn(
          'CharUtils.coseKeyToStorage: missing -2 or -3 key in Map',
          {
      ...
      x =
        obj[-2] !== undefined
          ? obj[-2]
          : (obj as { get?: (k: number) => unknown }).get?.(-2)
      y =
        obj[-3] !== undefined
          ? obj[-3]
          : (obj as { get?: (k: number) => unknown }).get?.(-3)

      if (!x || !y) {
        console.warn(
          'CharUtils.coseKeyToStorage: missing -2 or -3 key in object',
          {
            hasMinus2: (-2) in obj,
            hasMinus3: (-3) in obj,
      ...
      console.warn(
        'CharUtils.coseKeyToStorage: invalid COSE key format',
        typeof coseKey
      )
      return null
    }
    ...
    if (!x || !y) {
      console.warn(
        'CharUtils.coseKeyToStorage: COSE key missing x or y coordinate',
        {
          hasX: !!x,
          hasY: !!y,
          xType: typeof x,
    ...
    if (!xArray || !yArray) {
      console.warn(
        'CharUtils.coseKeyToStorage: failed to convert x or y coordinate to array',
        {
          xType: typeof x,
  ...
  static coseKeyFromStorage(
    storageData: CoseStorageFormat | CoseKey | null | undefined
    ...
    if (storageData instanceof Map) {
      if (storageData.has(-2) && storageData.has(-3)) {
        ...
        console.warn(
          'CharUtils.coseKeyFromStorage: invalid Map format (missing -2 or -3 key)'
        )
        return null
      }
    }
    ...
    if (typeof storageData === 'object' && storageData !== null) {
      ...
      if (keys.length === 0) {
        console.warn(
          'CharUtils.coseKeyFromStorage: storageData is an empty object {}'
        )
        return null
      }
    }
    ...
    if (typeof storageData === 'object' && storageData !== null) {
      if ((obj as unknown as CoseStorageFormat)._type === 'Map') {
        ...
        if (typed.x && typed.y) {
          ...
          yArray = typed.y
          DbgLog.log(
            'CharUtils.coseKeyFromStorage: using new format {_type: "Map", x, y}'
          )
        } else {
          console.warn(
            'CharUtils.coseKeyFromStorage: _type="Map" but missing x or y',
            {
              hasX: !!typed.x,
              hasY: !!typed.y,
        ...
        (obj as { x?: unknown; y?: unknown }).x !== undefined &&
        (obj as { x?: unknown; y?: unknown }).y !== undefined
      ) {
        ...
        DbgLog.log(
          'CharUtils.coseKeyFromStorage: using numeric-key format {-2, -3}'
        )
      ...
      console.warn(
        'CharUtils.coseKeyFromStorage: storageData is not an object',
        typeof storageData
      )
    }
    ...
    if (!xArray || !yArray) {
      console.warn(
        'CharUtils.coseKeyFromStorage: failed to extract x or y coordinate from storage data'
      )
    ...
    if (!Array.isArray(xArray)) {
      ...
      if (converted) {
        ...
        console.warn(
          'CharUtils.coseKeyFromStorage: x is not an array and cannot be converted',
          typeof xArray,
          xArray
        )
        return null
    ...
    if (!Array.isArray(yArray)) {
      ...
      if (converted) {
        ...
        console.warn(
          'CharUtils.coseKeyFromStorage: y is not an array and cannot be converted',
          typeof yArray,
          yArray
        )
        return null
    ...
    if (!xBytes || !yBytes) {
      console.warn(
        'CharUtils.coseKeyFromStorage: failed to convert arrays to Uint8Array',
        {
    ...
    if (xBytes.length !== 32 || yBytes.length !== 32) {
      console.warn(
        'CharUtils.coseKeyFromStorage: unexpected coordinate length (expected 32 bytes)',
        {
  ...
  static extractCoseKeyCoordinates(
    coseKey: CoseKey | CoseStorageFormat | null | undefined
  ): CoseCoordinates | null {
  ...
  static normalizeCoseKey(
    publicKey: CoseKey | CoseStorageFormat | null | undefined
  ): Map<number, Uint8Array> | null {
    ...
    console.warn(
      'CharUtils.normalizeCoseKey: failed to normalize publicKey format',
      publicKey
    )
    return null
  }

  static findPublicKeyFromStorage(
    credentialId: string | ByteLike | null | undefined,
    prefix = 'new_wallet_pk_'
  ): Map<number, Uint8Array> | null {
    ...
    if (relatedKeys.length > 0) {
      ...
      if (stored) {
        try {
          ...
          if (restored) {
            DbgLog.log(
              `✅ Found Public Key (fallback key: ${firstKey.substring(0, 20)}...)`
            )
            return restored
          }
        } catch (e) {
  ...
  static arrayBufferToUint8Array(
  ...
  static uint8ArrayToArrayBuffer(
    data: ByteLike | null | undefined
  ): ArrayBuffer | null {
    const arr = this.arrayToUint8Array(data)
    if (!arr) return null
