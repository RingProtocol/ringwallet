...
export async function upgradeTo7951({
  user,
  login,
}: UpgradeParams): Promise<UpgradeResult> {
  if (!user.ringsecurity_seedReady || !user.name) {
    return {
      success: false,
      error: 'Upgrade failed: missing required user info',
    }
  }
  ...
  if (!availability.isSecureContext)
    return {
      success: false,
      error: 'Passkey requires a secure context (HTTPS).',
    }
  ...
  if (!availability.isUVPAAAvailable)
    return {
      success: false,
      error:
        'No platform authenticator available (Touch ID / Face ID). Passkey cannot be used.',
    }
  ...
  try {
    if (seed.length !== 32) {
      throw new Error(
        `Invalid master seed length: expected 32 bytes, got ${seed.length} bytes`
      )
    }
  ...
  const credentialIdBase64 = CharUtils.uint8ArrayToBase64(
    new Uint8Array(result.credential.rawId)
  )
  if (!credentialIdBase64) {
    return {
      success: false,
      error: 'Upgrade failed due to invalid data format. Please try again.',
    }
  }
  ...
  if (!storedKey) {
    return {
      success: false,
      error: 'Upgrade failed: Public Key not found. Please try again.',
    }
  }
  ...
  if (!newPublicKey) {
    return {
      success: false,
      error: 'Upgrade failed: invalid Public Key format. Please try again.',
    }
  }
