...
const profile = getChainProfile('plasma-testnet')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_PLASMA_TESTNET?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_PLASMA_TESTNET=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe('src/ services on local Plasma Testnet (Anvil)', () => {
