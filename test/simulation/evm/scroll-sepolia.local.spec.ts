...
const profile = getChainProfile('scroll-sepolia')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_SCROLL_SEPOLIA?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_SCROLL_SEPOLIA=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Scroll Sepolia (Anvil)`, () => {
