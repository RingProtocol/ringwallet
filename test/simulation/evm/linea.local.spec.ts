...
const profile = getChainProfile('linea')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_LINEA?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_LINEA=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Linea (Anvil)`, () => {
