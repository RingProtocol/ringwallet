...
const profile = getChainProfile('fuji')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_FUJI?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_FUJI=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe('src/ services on local Avalanche Fuji (Anvil)', () => {
