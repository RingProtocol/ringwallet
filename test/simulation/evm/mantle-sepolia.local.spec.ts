...
const profile = getChainProfile('mantle-sepolia')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_MANTLE_SEPOLIA?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_MANTLE_SEPOLIA=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Mantle Sepolia (Anvil)`, () => {
