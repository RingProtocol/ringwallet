...
const profile = getChainProfile('fantom')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_FANTOM?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_FANTOM=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Fantom (Anvil)`, () => {
