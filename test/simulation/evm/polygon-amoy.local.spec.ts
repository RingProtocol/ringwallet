...
const profile = getChainProfile('polygon-amoy')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_POLYGON_AMOY?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_POLYGON_AMOY=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Polygon Amoy (Anvil)`, () => {
