...
const profile = getChainProfile('zora')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_ZORA?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_ZORA=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Zora (Anvil)`, () => {
