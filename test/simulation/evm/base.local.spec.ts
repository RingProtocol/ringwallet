...
const profile = getChainProfile('base')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_BASE?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_BASE=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Base (Anvil)`, () => {
