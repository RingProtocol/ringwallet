...
function forkHint(): string {
  ...
  try {
    url = profile.buildForkRpcUrl() ?? url
  } catch {
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  it('Anvil dev account #0 has a large ETH balance on the fork', async () => {
    ...
    console.log(
      `[sepolia] Anvil dev account[${wallet.address}] balance:`,
      bal.toString()
    )
  ...
  it('transfers 2 USDC from dev account #0 to #1 and verifies balance', async () => {
    ...
    const provider = new ethers.JsonRpcProvider(rpcUrl)
    const sender = new ethers.Wallet(ANVIL_DEV_PRIVATE_KEY, provider)
    // Anvil dev account #1
    const receiver = '0x70997970C51812dc3A010C7d01b50e0d17dc79C8'
    const amount = 2_000_000n // 2 USDC (6 decimals)

    // Mint USDC to sender via anvil_setStorageAt (slot 9 is balanceOf mapping for Sepolia USDC)
    const senderSlot = ethers.keccak256(
    ...
    await rpcCall(rpcUrl, 'anvil_setStorageAt', [
      usdc,
      senderSlot,
      ethers.zeroPadValue(ethers.toBeHex(100_000_000n), 32), // 100 USDC
    ])

    // Read receiver balance before transfer
    const balBeforeHex = await rpcCall<string>(rpcUrl, 'eth_call', [
      { to: usdc, data: erc20.encodeFunctionData('balanceOf', [receiver]) },
      'latest',
    ])
    const [balBefore] = erc20.decodeFunctionResult('balanceOf', balBeforeHex)

    // Transfer 2 USDC
    const tx = await sender.sendTransaction({
      to: usdc,
      data: erc20.encodeFunctionData('transfer', [receiver, amount]),
    })
    const receipt = await tx.wait()
    expect(receipt?.status).toBe(1)

    // Verify receiver balance increased by exactly 2 USDC
    const balAfterHex = await rpcCall<string>(rpcUrl, 'eth_call', [
      { to: usdc, data: erc20.encodeFunctionData('balanceOf', [receiver]) },
      'latest',
    ])
    const [balAfter] = erc20.decodeFunctionResult('balanceOf', balAfterHex)
