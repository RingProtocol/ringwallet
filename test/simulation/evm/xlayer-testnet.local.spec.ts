...
const profile = getChainProfile('xlayer-testnet')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_XLAYER_TESTNET?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_XLAYER_TESTNET=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe('src/ services on local X Layer Testnet (Anvil)', () => {
