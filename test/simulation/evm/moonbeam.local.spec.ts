...
const profile = getChainProfile('moonbeam')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_MOONBEAM?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_MOONBEAM=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Moonbeam (Anvil)`, () => {
