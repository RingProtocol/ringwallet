...
const profile = getChainProfile('scroll')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_SCROLL?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_SCROLL=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Scroll (Anvil)`, () => {
