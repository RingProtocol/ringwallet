...
const profile = getChainProfile('gnosis-chiado')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_GNOSIS_CHIADO?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_GNOSIS_CHIADO=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Gnosis Chiado (Anvil)`, () => {
