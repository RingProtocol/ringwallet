...
const profile = getChainProfile('celo')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_CELO?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_CELO=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Celo (Anvil)`, () => {
