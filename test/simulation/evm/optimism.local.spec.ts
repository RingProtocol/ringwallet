...
const profile = getChainProfile('optimism')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_OPTIMISM?.trim() ||
  `http://127.0.0.1:${profile.defaultAnvilPort}`

function optimismLocalChain(): Chain {
  return {
    id: profile.chainId,
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_OPTIMISM=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe('src/ services on local Optimism (Anvil)', () => {
    it('getPrimaryRpcUrl resolves the local HTTP endpoint from Chain', () => {
      expect(getPrimaryRpcUrl(optimismLocalChain())).toBe(rpcUrl)
    })
    ...
    it('EvmChainPlugin: derive masterSeed → sign EIP-1559 tx → broadcast', async () => {
      ...
      const chainConfig = optimismLocalChain()
