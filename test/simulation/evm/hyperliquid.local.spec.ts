...
const profile = getChainProfile('hyperliquid')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_HYPERLIQUID?.trim() ||
  `http://127.0.0.1:${profile.defaultAnvilPort}`

function hyperliquidForkChain(): Chain {
  return {
    id: profile.chainId,
...
function localHint(): string {
  return [
    ...
    `  anvil --chain-id ${profile.chainId} --port ${profile.defaultAnvilPort}`,
    '',
    'Or run yarn test:prepare to start all local chains at once.',
    'Override port: TESTCHAIN_RPC_URL_HYPERLIQUID=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  it('Anvil dev account #0 has a large balance on the fork', async () => {
  ...
  describe('src/ services on forked Hyperliquid (Anvil)', () => {
    it('getPrimaryRpcUrl resolves the fork HTTP endpoint from Chain', () => {
      expect(getPrimaryRpcUrl(hyperliquidForkChain())).toBe(rpcUrl)
    })
    ...
    it('EvmChainPlugin: derive masterSeed → sign EIP-1559 tx → broadcast', async () => {
      ...
      const chainConfig = hyperliquidForkChain()
