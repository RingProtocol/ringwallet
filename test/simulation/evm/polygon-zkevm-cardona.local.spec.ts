...
const profile = getChainProfile('polygon-zkevm-cardona')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_POLYGON_ZKEVM_CARDONA?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_POLYGON_ZKEVM_CARDONA=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Polygon zkEVM Cardona (Anvil)`, () => {
