...
const profile = getChainProfile('celo-alfajores')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_CELO_ALFAJORES?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_CELO_ALFAJORES=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Celo Alfajores (Anvil)`, () => {
