...
const profile = getChainProfile('zksync-sepolia')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_ZKSYNC_SEPOLIA?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_ZKSYNC_SEPOLIA=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local zkSync Sepolia (Anvil)`, () => {
