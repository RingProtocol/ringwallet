...
const profile = getChainProfile('gnosis')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_GNOSIS?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_GNOSIS=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Gnosis (Anvil)`, () => {
