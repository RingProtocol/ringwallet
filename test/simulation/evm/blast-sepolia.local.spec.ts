...
const profile = getChainProfile('blast-sepolia')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_BLAST_SEPOLIA?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_BLAST_SEPOLIA=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Blast Sepolia (Anvil)`, () => {
