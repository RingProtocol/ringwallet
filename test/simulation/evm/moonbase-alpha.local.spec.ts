...
const profile = getChainProfile('moonbase-alpha')
const rpcUrl =
  process.env.TESTCHAIN_RPC_URL_MOONBASE_ALPHA?.trim() ||
...
function localHint(): string {
  return [
    ...
    'Override port: TESTCHAIN_RPC_URL_MOONBASE_ALPHA=http://127.0.0.1:<port>',
  ].join('\n')
}
...
describe(`testchain: ${profile.id} (${profile.displayName})`, () => {
  ...
  describe(`src/ services on local Moonbase Alpha (Anvil)`, () => {
